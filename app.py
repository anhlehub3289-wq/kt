...
# Kiểm tra dữ liệu để render các phần biểu đồ và thuật toán
if df_raw is not None:
    ...
    with tab1:
        ...
        with chart_col1:
            ...
            fig_hour = px.bar(hour_counts, x='Giờ', y='Số lượng', labels={'Giờ': 'Giờ trong ngày', 'Số lượng': 'Số lượng giao dịch'}, color_discrete_sequence=['#2563EB'])
            fig_hour.update_layout(xaxis=dict(tickmode='linear', tick0=0, dtick=1))
            st.plotly_chart(fig_hour, use_container_width=True)
            
        with chart_col2:
            st.subheader("Phân phối số tiền giao dịch")
            fig_box = px.box(df, y='amount', log_y=True, labels={'amount': 'Số tiền giao dịch (Log scale)'}, color_discrete_sequence=['#10B981'])
    ...
    with tab2:
        ...
        fig_scatter = px.scatter(df, x='gio_giao_dich', y='amount', color='is_anomaly', color_discrete_map={False: '#3B82F6', True: '#EF4444'}, labels={'gio_giao_dich': 'Giờ thực hiện', 'amount': 'Số tiền giao dịch (VND)', 'is_anomaly': 'Bất thường?'}, hover_data=['transaction_id', 'location', 'is_employee'], title="Tương quan giữa Giờ giao dịch và Số tiền giao dịch (Đỏ: Bất thường)")
    ...
    with tab3:
        ...
        if len(df_bat_thuong) > 0:
            ...
            def assign_risk_level(score):
                if score < q25: return "Rủi ro khẩn cấp"
                elif score < q50: return "Rủi ro cao"
                elif score < q75: return "Rủi ro trung bình"
                else: return "Rủi ro thấp"
            ...
            st.write("---")
            selected_risk = st.selectbox("Chọn cấp độ rủi ro cần xem chi tiết và tải báo cáo:", options=["Rủi ro khẩn cấp", "Rủi ro cao", "Rủi ro trung bình", "Rủi ro thấp"], index=0)
            ...
            with btn_col1:
                st.download_button(label=f"📥 Tải Excel ({selected_risk})", data=excel_data, file_name=f"bao_cao_{selected_risk.lower().replace(' ', '_')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
            with btn_col2:
                st.download_button(label=f"📥 Tải CSV ({selected_risk})", data=csv_data, file_name=f"bao_cao_{selected_risk.lower().replace(' ', '_')}.csv", mime="text/csv", use_container_width=True)
        else:
            st.info("Không phát hiện giao dịch bất thường nào phù hợp với tham số hiện tại.")
else:
    # Đoạn báo lỗi dữ liệu giờ được đẩy xuống dưới tiêu đề
    st.warning("⚠️ Chưa nhận được tệp dữ liệu hoặc tệp mẫu `transactions_Q1_demo.csv` bị thiếu. Hãy tải lên tệp giao dịch (.csv) tại Sidebar để hệ thống bắt đầu phân tích.")
